...
// รับค่า chapter_id จาก GET/POST (ตกแต่งให้เป็นตัวเลข)
$chapter_id = isset($_GET['idc']) ? (int)$_GET['idc'] :

// เตรียมค่าเริ่มต้น
$firstVideo = null;
//echo $chapter_id;
// ถ้าระบุ chapter_id มา
if ($chapter_id > 0) {
  $sql="SELECT * FROM exercises1_video WHERE id_c = ? ORDER BY id DESC LIMIT 1";
  //echo $sql;
...
// ถ้าไม่เจอจาก chapter_id ให้ fallback เป็นวิดีโอล่าสุดทั้งหมด
if (!$firstVideo) {
    $vres = $mysqli->query("SELECT * FROM exercises1_video ORDER BY id DESC LIMIT 1");
    $firstVideo = $vres ? ($vres->fetch_assoc() ?: null) : null;
}

// สร้าง path ไฟล์วิดีโอ
$video_path = "videos/" . ($firstVideo ? htmlspecialchars($firstVideo['filename']) : 'default.mp4');




// ------- ดึงคำถามทั้งหมด (ถ้าจะแยกตามวิดีโอ แนะนำเพิ่มคอลัมน์ video_id และ WHERE video_id=... ) -------
$questions = [];
$qres = $mysqli->query("SELECT * FROM exercises1_questions ORDER BY timecode ASC");
if ($qres) {
...
video.addEventListener('timeupdate', function () {
  if (
    ...
    Math.floor(video.currentTime) >= toSec(questions[currentQ].timecode) &&
    getComputedStyle(questionBox).display === 'none'
  ) {
    video.pause();
...
document.getElementById('submit-answer').addEventListener('click', function() {
  ...
  if (selected) {
    ...
    // (ออปชัน) ถ้าตอบครบทุกข้อแล้ว บันทึกล่วงหน้าได้
    // if (currentQ >= questions.length && !scoreSaved) {
    //   document.getElementById('score_to_save').value = score;
    //   saveScore('all-answered');
    // }
  } else {
...
/* ---------- จบวิดีโอ: โชว์คะแนน + บันทึกอัตโนมัติ ---------- */
video.addEventListener('ended', function () {
  ...
  if (btn) btn.style.display = 'none'; // แสดงเฉพาะกรณี error

  saveScore('ended'); // บันทึกคะแนนอัตโนมัติ
});

/* ---------- ฟังก์ชันบันทึกคะแนน (ใช้ได้ทั้งอัตโนมัติ/กดเอง) ---------- */
function saveScore(reason = 'auto') {
  ...
  formData.append('save_reason', reason); // optional

  fetch(form.action, { method: 'POST', body: formData })
    .then(async (resp) => {
      const text = await resp.text();           // << อ่านข้อความจากเซิร์ฟเวอร์
    ...
    .catch((err) => {
      statusP.innerText = 'บันทึกไม่สำเร็จ: ' + err.message;  // << จะเห็น error จริง
      statusP.style.color = 'red';
      if (btn) btn.style.display = 'inline-block';
    });
}

/* ---------- Fallback: ให้ปุ่มในฟอร์มเรียก saveScore() ---------- */
document.getElementById('score-form').addEventListener('submit', function (e) {
  e.preventDefault();
  saveScore('manual');
});

/* ---------- เพลย์ลิสต์: เปลี่ยนวิดีโอ + รีเซ็ตสถานะควิซ ---------- */
